/**
 * Plantilla MAXIRent v4 — con imágenes reales de header/footer/página2
 * Variables: {{name}} = nombre del item Monday (empresa cliente)
 *
 * Ejecutar:  node seed_maxirent_v4.mjs
 */

const EMPTY = 'W10=' // btoa('[]')

const HDR_IMG  = 'https://analy-sys.pro/wp-content/uploads/2026/05/PRES_cotizacion_update-01.png'
const FTR_IMG  = 'https://analy-sys.pro/wp-content/uploads/2026/05/PRES_cotizacion_update-03.png'
const PG2_IMG  = 'https://analy-sys.pro/wp-content/uploads/2026/05/PRES_cotizacion_update-02.png'
...
<div class="prop">
  ...
  <!-- ══ HEADER — imagen real ══════════════════════════════════ -->
  <img src="${HDR_IMG}" style="width:100%;display:block;" alt="MAXIRent Propuesta Comercial" />
  ...
  <!-- ══ FOOTER — imagen real ══════════════════════════════════ -->
  <img src="${FTR_IMG}" style="width:100%;display:block;margin-top:16px;" alt="MAXIRent Footer" />

  <!-- ══ PÁGINA 2 — Perfil de empresa ══════════════════════════ -->
  <div class="page2">
    <img src="${PG2_IMG}" style="width:100%;display:block;" alt="MAXIRent Perfil Empresarial" />
  </div>

</div><!-- /prop -->
`

// ── Crear plantilla en el backend ──────────────────────────────
const res = await fetch('http://localhost:3001/api/templates', {
  ...
  body: JSON.stringify({
    ...
    description:  'Plantilla oficial con header/footer reales, 3 tablas interactivas y página 2 de perfil empresarial.',
